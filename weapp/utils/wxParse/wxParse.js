...
var _html2json = _interopRequireDefault(require('html2json.js'));
...
function wxParse() {
  ...
  var imagePadding = arguments.length > 4 ? arguments[4] : undefined;
  var that = target;
  var transData = {}; //存放转化后的数据

  if (type == 'html') {
    transData = _html2json["default"].html2json(data, bindName); // console.log(JSON.stringify(transData, ' ', ' '));
  ...
  return transData; // console.log(transData);
...
function wxParseImgTap(e) {
  ...
  if (typeof tagFrom != 'undefined' && tagFrom.length > 0) {
    var obj = that.data[tagFrom].imageUrls;
    var arr = [];
...
function calMoreImageInfo(e, idx, that, bindName) {
  ...
  var bindData = {}; // bindData[bindName] = temData;

  that[bindName] = temData;
  that.setData(bindData);
} // 计算视觉优先的图片宽高
