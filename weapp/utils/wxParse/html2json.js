  ...
  HTMLParser(html, {
    start: function start(tag, attrs, unary) {
      ...
      if (node.tag === 'img') {
        ...
        // console.log(getApp().globalData.deviceinfo.system.indexOf("Android")!==-1);
        node.imgIndex = results.images.length;
        node.bigimgurl = node.attr.src;
        var imgUrl = node.attr.src; // jpg正则

        var reg = /[http|https]:\/\/.*?(jpg)/gi;
        imgUrl = wxDiscode.urlToHttpUrl(imgUrl, __placeImgeUrlHttps); // 正则匹配图片地址

        if (reg.test(imgUrl)) {
          //不包含其他url参数
          if (imgUrl.indexOf('?') == -1) {
            imgUrl += '?x-oss-process=image/resize,m_lfit,w_800';
          } else {
            imgUrl += '&x-oss-process=image/resize,m_lfit,w_800';
          }
        } //   if (getApp().globalData.deviceinfo.system.indexOf("Android") != -1) {
